...
export const generateAddressFromSeed = (encryptedSeed, network, password) => {
  ...
  if (!network||network=="LTC" ||network==litecoinNetwork) {
    network = 'litecoin';
  }
  console.log('network in gen addr '+network)
  const selectedNetwork = network === 'litecoin' ? litecoinNetwork : litecoinTestnet;
  console.log('selected network in address re-derivation '+JSON.stringify(selectedNetwork))
