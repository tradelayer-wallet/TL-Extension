...
export const generateAddressFromSeed = (seedPhrase, network, password) => {
  console.log('Seed:', seedPhrase);
  if (!network) {
    network = 'litecoin';
  }
  const selectedNetwork = network === 'litecoin' ? litecoinNetwork : litecoinTestnet;

  let path;
  if (selectedNetwork.bech32 === 'ltc') {
    path = "m/84'/2'/0'/0"; // Litecoin mainnet
  } else if (selectedNetwork.bech32 === 'tltc') {
    path = "m/84'/1'/0'/0"; // Litecoin testnet
  } else {
  ...
  try {
    ...
    console.log('Valid mnemonic?', bip39.validateMnemonic(seedPhrase)); // should be true
...
export async function checkPasswordMatch(password, network, expectedAddress) {
  try {
    ...
    const networkType = network === "LTCTEST" ? 'litecoinTestnet' : 'litecoin';
    console.log('Network for address derivation:', networkType);

    // Generate the address from the seed
    const derivedAddress = generateAddressFromSeed(serializedSeed, networkType,password);
...
export async function getOrDeriveKeyPair(password, network) {
  ...
  if (!encryptedPrivateKey) {
    ...
    let address = generateAddressFromSeed(serializedSeed, network,password);
    serializedSeed = null
    password = null
