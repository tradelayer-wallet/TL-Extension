...
export const signTransaction = async (partialRawHex, password, finalize =true) => {
  //try {
  ...
  // Suppose privateKey is "173,16,104,182,..." in decimal form
  const privKeyArray = privateKey.split(',').map(str => parseInt(str.trim(), 10));
  const keyBuffer = Buffer.from(privKeyArray);

  // Confirm it's 32 bytes
  if (keyBuffer.length !== 32) {
    throw new Error(`Invalid private key buffer length: ${keyBuffer.length}`);
  }

  // 2) Create the key pair from the raw buffer (NOT hex!)
  const keyPair = ECPair.fromPrivateKey(keyBuffer);
  const publicKey = keyPair.publicKey.toString('hex');

  // Example: Derive a SegWit (P2WPKH) address
const publicKeyBuffer = Buffer.from(keyPair.publicKey); // Ensure it's a Buffer
const { address } = bitcoin.payments.p2wpkh({
  pubkey: publicKeyBuffer, // Pass the converted Buffer
  network: litecoinTestnet, // Custom Litecoin testnet network
});
console.log('Litecoin Testnet Address:', address);


    console.log('checking tx hex '+partialRawHex)
  const psbt = bitcoin.Psbt.fromBase64(partialRawHex);
console.log('PSBT inputs:', psbt.data.inputs);


psbt.data.inputs.forEach((input, index) => {
  const { script } = input.witnessUtxo; // Or redeemScript for P2SH
  ...
  if (inputAddress === address) {
    console.log(`Private key can sign Input ${index}`);
  } else {
    console.log(`Private key does not match Input ${index}`);
  }
});
