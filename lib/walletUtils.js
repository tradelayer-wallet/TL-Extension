...
export const generateSalt = (password) => {
  return CryptoJS.SHA256(password + Date.now().toString()).toString();
};

export const storeSalt = (salt) => {
  const encryptedSalt = CryptoJS.AES.encrypt(salt, 'extension-secret-key').toString();
  localStorage.setItem('salt', encryptedSalt);
};

export const retrieveSalt = () => {
  const encryptedSalt = localStorage.getItem('salt');
  if (!encryptedSalt) return null;

  const bytes = CryptoJS.AES.decrypt(encryptedSalt, 'extension-secret-key');
  return bytes.toString(CryptoJS.enc.Utf8);
};

export const preAuthorizeTransaction = (transactionParams) => {
  const salt = retrieveSalt();
  if (!salt) throw new Error('Salt not available. User must log in.');

  const preAuthorizedScope = {
    token: transactionParams.token,
    maxAmount: transactionParams.maxAmount,
    maxGasFee: transactionParams.maxGasFee,
    expiration: Date.now() + 3600000, // Expires in 1 hour
  };

  localStorage.setItem(
    'preAuthorizedScope',
    CryptoJS.AES.encrypt(JSON.stringify(preAuthorizedScope), salt).toString()
  );
};

export const isTransactionAuthorized = (transaction) => {
  const salt = retrieveSalt();
  const encryptedScope = localStorage.getItem('preAuthorizedScope');
  if (!salt || !encryptedScope) return false;

  const bytes = CryptoJS.AES.decrypt(encryptedScope, salt);
  const preAuthorizedScope = JSON.parse(bytes.toString(CryptoJS.enc.Utf8));

  return (
    transaction.token === preAuthorizedScope.token &&
    transaction.amount <= preAuthorizedScope.maxAmount &&
    transaction.gasFee <= preAuthorizedScope.maxGasFee &&
    Date.now() < preAuthorizedScope.expiration
  );
};

export const autoSignTransaction = (transaction, encryptedPrivateKey) => {
  if (!isTransactionAuthorized(transaction)) {
    throw new Error('Transaction is not pre-authorized.');
  }

  const salt = retrieveSalt();
  const privateKeyBytes = CryptoJS.AES.decrypt(encryptedPrivateKey, salt);
  const privateKey = privateKeyBytes.toString(CryptoJS.enc.Utf8);

  const signature = signString(privateKey, transaction.hash);

  privateKeyBytes.fill(0); // Clear sensitive data from memory
  return signature;
};


export const signWithDecryptedKey = (encryptedKey, salt, transactionHash) => {
  const decryptedKey = decryptPrivateKey(encryptedKey, salt);
  ...
  try {
    // Sign the transaction or message
    const signature = signString(decryptedKey, transactionHash); // Assume signString is your ECDSA signing function
