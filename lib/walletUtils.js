...
export const signInputsInPsbt = async (
  ...
  try {
    console.log('Starting PSBT signing process...');

    // Step 1: Reinitialize PSBT from provided hex
    const psbt = bitcoin.Psbt.fromHex(psbtHex, { network });
    console.log('Decoded PSBT:', JSON.stringify(psbt.data, null, 2));
    ...
    psbt.data.inputs.forEach((input, index) => {
      ...
      if (input.witnessScript) {
        console.log(`Input ${index} is a multisig input.`);
        try {
          psbt.signInput(index, keyPair);
        } catch (err) {
          console.error(`Failed to sign multisig input ${index}:`, err);
        }
      } else {
        console.log(`Input ${index} is a standard input.`);
        try {
          psbt.signInput(index, keyPair);
        } catch (err) {
          console.error(`Failed to sign standard input ${index}:`, err);
        }
      }
    });
    ...
    // Step 5: Finalize inputs if signatures are valid
    if (allSignaturesValid) {
      ...
      console.error('Some signatures are invalid. Cannot finalize PSBT.');
      throw new Error('Invalid signatures in PSBT.');
    }

    // Step 6: Extract the final transaction
    const finalTx = psbt.extractTransaction();
    const rawTx = finalTx.toHex();
    const txid = finalTx.getId();
