...
export const generateAddressFromSeed = (seedPhrase, network) => {
    ...
    if(network=='litecoin'){test=litecoinNetwork
    }else if(network=='litecoin-testnet'){test=litecoinTestnet}
  ...
  try {
    ...
    let hdNode = bip32.fromSeed(seed, test);

    // Derive the keypair based on the provided path
    console.log('about to derive node for path '+path)
       let derivedNode = hdNode.derivePath(path); // BIP44 path for Litecoin
...
export function addMultisigAddress(m, pubkeys, network) {
  ...
  localStorage.setItem('multisigs', JSON.stringify(multisigs));
   chrome.storage.local.set({ multisigs }, () => {
        console.log('Multisig Address saved to storage:', { multisigs });
        return {
...
export async function checkPasswordMatch(password, network, expectedAddress) {
  ...
  //try {
      // If no encrypted private key is found, try re-deriving from the seed
      const encryptedSeed = localStorage.getItem('encryptedSeed');
      ...
      console.log('network in check password '+network)
      let test = network === "LTCTEST" ? 'litecoinTestnet' : 'litecoin';
      // Generate the address from the seed
      let derivedAddress = generateAddressFromSeed(serializedSeed, test);
      serializedSeed = null; // Clear sensitive data

      if (!derivedAddress || !derivedAddress.publicAddress) {
        throw new Error('Failed to derive address from seed.');
      }
      console.log('derivedAddress '+JSON.stringify(derivedAddress))
...
export async function getOrDeriveKeyPair(password, network) {
  ...
  if (!encryptedPrivateKey) {
    ...
    let address = generateAddressFromSeed(serializedSeed, network);
    serializedSeed = null
    ...
    let privateKeyBase64 = Buffer.from(address.privateKey).toString('base64'); // Base64 encoding
...
export async function signTransaction(requestPayload, password, network) {
  //try {
    ...
    let keyPair = await getOrDeriveKeyPair(password, test);
